...
TEST(BplusTreeLogEntry, init_header_page_log_entry)
{
  ...
  file_header.attr_types.push_back(AttrType::INTS);
  ...
  ASSERT_EQ(file_header.attr_types.size(), file_header2.attr_types.size());
  for (size_t i = 0; i < file_header.attr_types.size(); i++) {
    ASSERT_EQ(file_header.attr_types[i], file_header2.attr_types[i]);
  }
