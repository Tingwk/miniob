...
const static Json::StaticString NULLABLE("nullable");
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field) {
  ...
  const Json::Value &nullable = json_value[NULLABLE];
  ...
  if (!nullable.isBool()) {
    LOG_ERROR("Field id is not a string. json value=%s", nullable.toStyledString().c_str());
    return RC::INTERNAL;
  }
  ...
  return field.init(name, type, offset, len, visible, field_id, nullable.asBool());
}
